...
// import Avatar from "@/Views/Player/Avatar/index";
import Player1 from "@/Views/Player/1Player";
import Player2 from "@/Views/Player/2Player";
import Player3 from "@/Views/Player/3Player";
function AppTour() {
  ...
  let currentAlgoDes = null;
  const currentAlgo = algorithms.find((item) => item.name == algo);
  if (currentAlgo) currentAlgoDes = currentAlgo?.description;
  else currentAlgoDes = algorithms[0].description;
  ...
  const tourSteps: Step[] = [
    {
      content: (
        <>
          <h2 style={{ margin: "0" }}>
            第{gameNumber}关：<span>{scene}</span>-<span>{algo}</span>
          </h2>
          <br />
          <div
            style={{
          ...
          <p style={{ fontSize: "14px" }}>算法介绍:{currentAlgoDes}</p>
        </>
      ),
    ...
    {
      content: (
        <h4 style={{ fontSize: "14px" }}>分析路径搜索的细节并进行比较</h4>
      ),
      target: ".path-info",
    },
  ];
